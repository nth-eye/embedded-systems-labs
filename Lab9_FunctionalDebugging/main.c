...
void Delay(unsigned long t) //unsigned long volatile time;
{
  while (t--);
}
...
int main(void)
{  
	unsigned long i,last,now, prev_state = 0, new_state = 0;
  ...
  while (1) {
		if (~GPIO_PORTF_DATA_R & 0x11) {
			...
			Delay(80000*2); // 0.1sec
		} else {
			GPIO_PORTF_DATA_R &= ~0x02;
		}
		
		new_state = GPIO_PORTF_DATA_R & 0x13;
		if (prev_state != new_state && i < 50) {
			now = NVIC_ST_CURRENT_R;
			Time[i] = (last-now) & 0x00FFFFFF;  // 24-bit time difference
			Data[i] = new_state; // record PF1
			last = now;
			i++;
